...
Основной язык сайта — украинский. Русская версия доступна под URL-префиксом `/ru/`.
...
| Hugo | `0.164.0` |
| Node.js | `24.16.0` |
| npm | `11.13.0`, поставляется с закрепленной Node.js |
| Tailwind CSS | `4.3.0` |
...
| `docs/SKY LITE/` | Исходные изображения производителя для SKY Lite; не публикуются Hugo |
| `docs/SKY 360/` | Исходные изображения производителя для SKY 360; не публикуются Hugo |
| `docs/WING 360/` | Исходные изображения производителя для серии WING 360; не публикуются Hugo |
...
### 5.1. Канонические названия товаров

Производитель и бренд подтвердили названия `Aerocool SKY Lite`, `Aerocool SKY 360`, серии `Aerocool XTAL` и `Aerocool WING 360`. В официальных названиях нельзя заменять `Lite` на `Light` или сокращать `WING 360` до `WING`.

Канонические маршруты серий и моделей:

- SKY Lite — `/products/sky/lite/`;
- SKY 360 — `/products/sky/360/`;
- XTAL — `/products/xtal/`;
- WING 360 — `/products/wing-360/`.

Полный перечень двенадцати товарных названий, цветов, URL и ID сущностей закреплен в `docs/seo/31-product-facts-maintenance-process-2026.md`.

## 6. Изображения

Текущий проверенный снимок на 2026-07-15:
...
1. `netlify/functions/reviews.mjs` принимает только `POST` для `target_type=product`, проверяет запрос и создает отзыв со статусом `pending`; чтением approved-записей занимается отдельный сборочный экспорт.
...
Все постоянные руководства имеют номера `01–41`. Аудиты `42–100` являются историческими снимками и читаются после рабочих руководств. Текущая итоговая проверка — `docs/audits/101-2026-07-13-full-documentation-project-sync-audit-current.md`.
