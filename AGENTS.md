...
- `docs/XTAL/`, `docs/SKY LITE/`, `docs/SKY 360/` и `docs/WING 360/` — Git-архивы исходных изображений производителя, не публичные папки Hugo и не часть нумерованной документации.
...
Канонические названия подтверждены производителем и брендом `2026-07-15`:

- `Aerocool SKY Lite`, цвет `Mesh Black`; URL товара — `/products/sky/lite/`, ID сущности — `sky-lite`;
- `Aerocool SKY 360`, цвет `Mesh Black`; URL товара — `/products/sky/360/`, ID сущности — `sky-360`;
...
- Текущий опубликованный контент использует WebP: на 2026-07-15 в `content/` проверено `188` WebP и `0` PNG/JPEG.
...
- Дополнительные товарные кадры должны быть обрабатываемыми Hugo растровыми ресурсами и иметь одно из имен, разрешенных `layouts/_partials/products/gallery-image-meta.html`; неизвестное имя останавливает сборку. SVG автоматически в галерею не попадает.
...
- Все новые генерации, композиты, ретушь и заказная съемка следуют визуальной системе "спокойная технологичность" из `docs/content/34-image-design-playbook-2026.md`: точный товар, понятная функция и сдержанный реальный сценарий. Официальные архивы `docs/XTAL/`, `docs/SKY LITE/`, `docs/SKY 360/` и `docs/WING 360/` имеют приоритет как референс; не использовать универсальный темный/neon high-tech шаблон, corner-logo, текст или watermark поверх изображения.
...
- `Product.material` формируется из подтвержденного поля `material` главной товарной сущности в `data/entities.yaml`; не дублировать его произвольной строкой во front matter.
- `Product.name` формируется из локализованного канонического `name` главной товарной сущности; расширенный редакционный `title` страницы не должен подменять официальное название в JSON-LD.
- `Product.color` формируется из подтвержденного поля `official_color` главной товарной сущности. Поле `color` хранит только нормализованный CSS-токен для цветовых свотчей; не подменять им официальное обозначение варианта.
...
- Детальные статьи показывают связанного автора-организацию «Редакция Aerocool Ukraine», дату и время чтения; имя автора ведет на редакционную политику в About.
- Карточки статей показывают дату и время чтения без повторения автора.
- Новости показывают только дату в основной meta-строке; автор-организация виден в редакционном блоке.
- Авторство статей и новостей задается централизованно через `layouts/_partials/editorial-author.html` и `data/entities.yaml`, а не повторяется во front matter.
...
- Перед production проверять обычные сценарии форм детерминированными тестами и отдельно выполнять агентные сценарии: выбор нужного инструмента, корректность параметров, подтверждение записи и обработку недоверенного содержимого.
- `static/llms.txt` поддерживать как краткую Markdown-карту со ссылками, а не как второй sitemap или рекламный текст.

Текущая профильная проверка: `docs/audits/96-2026-07-08-webmcp-llms-agentic-readiness-audit.md` с уточнениями от 2026-07-10 и 2026-07-13.
