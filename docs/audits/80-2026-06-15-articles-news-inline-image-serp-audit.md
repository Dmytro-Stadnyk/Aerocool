...
Статус: историческая матрица внедрения inline-изображений. Текущий глубокий аудит текстов и изображений находится в [82-2026-06-18-articles-news-content-image-audit.md](82-2026-06-18-articles-news-content-image-audit.md).
...
Документ заменил план [79-2026-06-14-articles-news-inline-image-plan.md](79-2026-06-14-articles-news-inline-image-plan.md) на этапе внедрения. Аудиты `73` и `74` теперь остаются историческими snapshot.
...
## 2. Текущее состояние на 2026-06-16
...
- у всех `16` статей внедрены дополнительные inline-изображения `02-*` / `03-*`;
- всего в `content/articles` есть `32` активных inline WebP-файла после обложек, все они имеют размер **1200x800** и вес примерно **61-216 КБ**;
- у всех `9` новостей внедрен второй визуальный слой после `01-front.webp`;
- всего в `content/news` есть `12` активных inline WebP-файлов после обложек, все они имеют размер **1200x800** и вес примерно **58-211 КБ**;
- текущий слой inline-иллюстраций нужно считать внедренным: production backlog по `content/articles` и `content/news` закрыт.

Оценка текущего visual content depth для `content/articles` и `content/news`: **9.1 / 10**.

Почему не выше:

- primary covers, schema crops и inline-слой закрыты системно;
- все evergreen-статьи и новости имеют второй визуальный уровень, связанный с конкретным блоком текста;
- остаются ручные риски visual QA: геометрия AI-кресел, бренд на материале, отсутствие случайного текста, однотипность сцен;
- один cover-файл немного выходит за целевой весовой диапазон и требует точечного переэкспорта только если визуальное качество сохранится;
- итоговую SEO-эффективность нужно подтверждать после деплоя через Google Search Console и PageSpeed Insights, а не только локальной проверкой.

Целевая оценка после ручной визуальной приемки и точечной оптимизации тяжелого cover: **9.3-9.5 / 10**, если изображения останутся фактически связаны с текстом, легкие, без случайного текста и с локализованным `alt`.
...
- если главный объект - кресло крупным планом, приоритет отдавать официальному product asset или аккуратному композиту с official cutout; полностью AI-кресло допустимо только после ручной проверки геометрии, бренда, колес, подлокотников и материалов;
- официальный `static/images/logo.svg` нельзя механически накладывать на подголовник как темную плашку, жирный контрастный знак или sticker-like overlay; если бренд не выглядит частью материала, изображение нужно перегенерировать или собрать на основе official product image;
- для inline-изображений corner logo обычно не нужен: он остается правилом cover/category/fallback слоя, а в теле статьи важнее понятная сцена и корректный бренд на самом кресле, если кресло достаточно крупное;
...
- [ ] если бренд на кресле крупно видим, он не выглядит как наклейка, прямоугольная плашка или отдельный overlay;
- [ ] если AI не справляется с брендом или геометрией, используется official product asset / cutout или изображение отклоняется;
...
Главный разрыв по inline-изображениям закрыт: обложки, schema crops и второй визуальный слой есть у всех статей и новостей. Новые обязательные картинки в текущем `content/articles` и `content/news` не нужны.

Дальше приоритет не в массовом добавлении файлов, а в контроле качества:

1. Ручная визуальная приемка всех новых `02-*` / `03-*` изображений.
2. Точечный переэкспорт только тех файлов, где найдены дефекты бренда, геометрии, текста, веса или однотипности.
3. Новые изображения добавлять только под новый смысловой блок, новый официальный product asset, новый инфоповод или реальные данные Search Console.

Текущая оценка `content/articles` и `content/news` по текстам, изображениям и SERP-пригодности: **9.1 / 10**.
