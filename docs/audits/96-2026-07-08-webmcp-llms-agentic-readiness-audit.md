...
Этот документ объясняет, что было добавлено для PageSpeed Agentic Browsing, зачем это нужно и как проверять результат. Он написан для новичка, который впервые встречает `toolname`, `tooldescription`, `toolparamdescription` или `llms.txt`.

> Статус: исторический профильный снимок от 2026-07-08, дополненный уточнениями 2026-07-10 и 2026-07-13. Оценка ниже относится к состоянию и методике на дату исходной проверки, а не является текущей полной оценкой проекта.
...
## Уточнение 2026-07-13

- Один Lighthouse или PageSpeed-прогон проверяет техническую доступность WebMCP, но не доказывает, что AI-агент всегда выберет правильный инструмент и завершит задачу.
- Перед production нужны отдельные сценарии: выбор формы контакта, фильтра или отзыва; корректность параметров; явное подтверждение записывающего действия; безопасная обработка внешнего и пользовательского содержимого.
- Детерминированные проверки обычных HTML-форм остаются обязательными и выполняются независимо от агентных сценариев.
- Автоматизированного набора WebMCP-сценариев в репозитории пока нет. До его появления результат фиксируется вручную по [production gate](../quality/14-production-quality-gate-2026.md).
- Источник: [Chrome WebMCP evals](https://developer.chrome.com/docs/ai/webmcp/evals).
...
- корневой `llms.txt` существует, написан как Markdown, имеет H1, дату обновления, читаемые короткие строки, ссылки на ключевые страницы, серии и все товарные страницы в украинской и русской версиях;
...
- ссылки на все `12` русских товарных страниц;
- ссылки на украинские и русские страницы серий `SKY`, `WING` и `XTAL`.
...
- `public/llms.txt` содержит H1, дату обновления, Markdown-ссылки и все `24` товарных URL: `12` украинских и `12` русских.
...
Дополнительная файловая проверка после расширения `llms.txt`:

- `12 / 12` украинских товарных URL есть в `static/llms.txt`;
