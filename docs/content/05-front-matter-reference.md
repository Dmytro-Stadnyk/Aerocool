...
Историческая проверка [аудита 89](../audits/89-2026-06-24-cover-block-image-seo-audit.md) зафиксировала `100/100` Markdown-файлов с полным `image` + `cover`-блоком и `25/25` пакетами article/news с наборами вариантов пропорций. Последняя полная проверка указана в [карте документации](../01-documentation-map.md#5-текущий-аудит). Этот контракт нужно сохранять при новых страницах и правках front matter.
