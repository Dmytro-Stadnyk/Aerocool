...
`related_series` — ручной список серий для блока “Что посмотреть дальше”. Использовать только значения `sky`, `wing-360`, `xtal`. Поле нужно тогда, когда страница явно связана с серией, но автоматический вывод по заголовку, описанию и entity-полям может быть слишком широким или слишком слабым.

`related_products` — ручной список товарных страниц, которые нужно показать первыми в блоке похожих моделей. Значения задаются как языконезависимые content paths без начального `/` и без `/ru/`, например `products/wing-360/racer-black`. Шаблон сам подставит URL текущего языка.
...
related_products:
  - "products/wing-360/racer-black"
  - "products/wing-360/loft-air-dark-grey"
...
cover:
  image: "cover.webp"
  alt: "Aerocool Україна: каталог крісел серій SKY, WING 360 і XTAL"
  relative: true
  hiddenInSingle: true
...
Для страниц серий `cover.image` нужен, если серия выводится карточкой внутри родительского каталога `/products/`. В текущем проекте это рекомендованный паттерн для `SKY`, `WING 360` и `XTAL`: `image` остается источником для SEO/OG/schema, а `cover.image` дает preview в листинге. `alt` должен быть локализован под язык страницы.
...
[layouts/_partials/_schema/product.html](../../layouts/_partials/_schema/product.html) читает коммерческие факты из front matter и строит `Offer`, `OfferShippingDetails`, `MerchantReturnPolicy`, `acceptedPaymentMethod` и `WarrantyPromise`. Канонический `Product.name` partial берет из локализованного `name` главной товарной сущности в [data/entities.yaml](../../data/entities.yaml), поэтому расширенный редакционный `title` страницы не подменяет официальное название товара. Из этой же сущности берутся официальное обозначение варианта `official_color` и ссылка на `material`; нормализованный `color` используется только CSS-свотчами. Для подтвержденной сущности материала partial записывает локализованное название в `Product.material`, а видимые `characteristics` преобразует в `Product.additionalProperty`. Видимый коммерческий блок товарной страницы и `/faq/` должны подтверждать коммерческие факты, но не являются первичным источником. При изменении цены, наличия, SKU, GTIN, гарантии, доставки, возврата или оплаты сначала обновляют front matter товара, затем в том же изменении синхронизируют видимый текст и `/faq/`, если меняется общая политика. При изменении названия, цвета или материала одновременно обновляют реестр сущностей и видимые данные. Сам partial меняют только при изменении сопоставления schema.org или добавлении новых полей.
...
linkTitle: "FAQ"
h1: ""          # необязательно
description: "FAQ Aerocool в Украине: серии SKY, WING 360 и XTAL, материалы Racer, Loft Air и Mesh, 7D, 8D, 11D, Synchronous Tilt, гарантия, доставка, оплата, возврат, сборка и выбор кресла для работы и гейминга."
  ...
  - title: "Выбор кресла и серии"
    items:
      - question: "Aerocool.ua — это официальный сайт бренда в Украине?"
        answer: "Да. Aerocool.ua — официальный сайт Aerocool в Украине. Здесь собраны актуальные серии SKY, WING 360 и XTAL, а также гайды, FAQ, новости и контакты."
      - question: "Чем отличаются серии SKY, WING 360 и XTAL?"
        answer: "Если коротко, SKY больше подходит для ежедневной работы и home office. WING 360 стоит выбирать, когда нужны более активная поддержка спины и более широкие возможности настройки. XTAL подойдет тем, кто ищет более спокойную эргономику, Synchronous Tilt и конструкцию со сменными элементами."
