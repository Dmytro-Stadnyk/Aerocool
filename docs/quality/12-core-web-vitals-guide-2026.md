...
Исторический Netlify RUM-срез за 2026-06-20—2026-06-21 показал статус `Good` по p75 для `LCP`, `INP` и `CLS`. Этот 24-часовой aggregate `All Devices` не описывает автоматически текущее поле и не заменяет 28-дневные данные CrUX/Search Console с разделением mobile/desktop. Подробности находятся в [аудите 86](../audits/86-2026-06-21-netlify-rum-core-web-vitals-baseline.md).
