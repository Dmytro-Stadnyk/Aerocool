...
Netlify Database доступна только на credit-based plans и расходует credits на compute и bandwidth. Официальная страница указывала бесплатное хранение данных только до 2026-07-01; эта дата уже прошла. Текущие условия проверены и приняты владельцем проекта `2026-07-14`, но это не бессрочная гарантия цены: проверку в Netlify Dashboard нужно повторять при изменении тарифа, цен, лимитов или ожидаемой нагрузки. Документация проекта не считает базу безусловно бесплатной и не переносит устаревшую акционную цену на текущий период.

### Подтвержденная ответственная роль

Уполномоченный администратор ТОВ «Aerocool» выполняет четыре обязательные функции:

1. Проверяет каждый pending-отзыв до перевода в `approved`; автоматическая публикация запрещена.
2. Сверяет `target_id`, URL, модель, язык, оценку и текст отзыва с реальным товаром.
3. Не реже одного раза в месяц проверяет сроки хранения (`retention`) и удаляет записи по правилам раздела 9.1.1.
4. Не реже одного раза в рабочий день проверяет отправки контактной формы (`submissions`) и точную версию согласия по [production quality gate](../quality/14-production-quality-gate-2026.md).

Если конкретный сотрудник меняется, ответственность остается за этой организационной ролью и должна быть передана без перерыва. Подтверждение роли и тарифа не заменяет отдельную техническую проверку `REVIEW_EMAIL_HASH_SALT` и миграций удаленной production-ветки Database.
...
Перед переводом в `approved` уполномоченный администратор ТОВ «Aerocool» обязан проверить соответствие `target_id`, URL, модели, языка, оценки и текста. Function проверяет формат полей и товарный путь, но не сверяет `target_id` с реальным каталогом автоматически.
...
Функция принимает только `POST`, читает `application/x-www-form-urlencoded`, `multipart/form-data` или `application/json`, проверяет ловушку для ботов `bot-field`, валидирует поля, обязательное согласие и точное значение `privacy_policy_version=2026-07-13-v2`, считает `author_email_hash` на сервере и записывает отзыв в `reviews` со статусом `pending`. Hidden-поле связывает форму с показанной редакцией политики, но в базу функция записывает собственную серверную константу и серверное время согласия, поэтому подмененное значение клиента не становится юридическим фактом. Любой другой HTTP-метод, включая `GET`, получает `405 Method Not Allowed` с `Allow: POST`; функция не читает и не публикует approved-отзывы.
...
Автоматическая задача контроля сроков хранения (`retention`) пока не реализована. До ее появления назначенный уполномоченный администратор ТОВ «Aerocool» не реже одного раза в месяц проверяет старые строки в Netlify Database и фиксирует удаление. Нельзя обещать срок в публичной политике и одновременно оставлять старые записи без операционной проверки.

Владелец проекта подтвердил `2026-07-13`, что владельцем персональных данных является `ТОВ «Aerocool»`. Редакция policy `2026-07-13-v2` фиксирует правовые основания, роль Netlify как обработчика, трансграничную передачу, сроки хранения и канал обращений. Техническая реализация согласия должна оставаться синхронной с этой редакцией; неподтвержденный ЕДРПОУ не добавляется.
