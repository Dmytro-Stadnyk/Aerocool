...
- Документы `42–98` — исторические аудиты. Их выводы верны для указанной в имени даты и не описывают автоматически текущее состояние.
- Документ `99` — единственный текущий полный аудит на 2026-07-10.
- CSV-файлы `30` и `32` являются приложениями к одноименным Markdown-документам и наследуют их номер.
- `docs/XTAL/` содержит исходные изображения производителя. Это архив ресурсов, а не документация, поэтому его папки и изображения не получают номера маршрута чтения.
...
## 4. Исторические аудиты: 42–98

Эти документы сохраняют ход решений и доказательства на конкретную дату. Не используйте слово `current` в старом английском имени как гарантию актуальности: текущим считается только аудит `99`.
...
96. `docs/audits/96-2026-07-08-webmcp-llms-agentic-readiness-audit.md` — профильный снимок WebMCP и `llms.txt`, дополненный уточнением 2026-07-10.
...
99. `docs/audits/99-2026-07-10-full-documentation-project-sync-audit-current.md` — текущая полная проверка структуры, языка, ссылок, фактов, сборки и синхронизации с проектом.
...
Читайте `20 -> 25 -> 27 -> 28 -> 36`, затем профильный аудит `96` с уточнением от 2026-07-10.
