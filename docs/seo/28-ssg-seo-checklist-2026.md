...
- [x] Утвержденный операционный процесс и назначенный ответственный проверяют согласие при прямом POST.
- [ ] Политика конфиденциальности на обоих языках доступна из формы и подвала.
- [x] Подтверждены контролер, правовое основание, сроки хранения и трансграничная передача.
...
- [x] Уполномоченный администратор ТОВ «Aerocool» проверяет `target_id` до перевода отзыва в `approved`.
- [x] Назначенный ответственный не реже одного раза в месяц проверяет сроки хранения (`retention`) в Netlify Database.
- [x] Лимиты, credits, compute, bandwidth и стоимость Netlify Database проверены в Dashboard и приняты владельцем проекта `2026-07-14`.

Текущее подтвержденное состояние privacy-слоя: владелец персональных данных — `ТОВ «Aerocool»`, версия policy и согласия — `2026-07-13-v2`, роль Netlify и трансграничная обработка описаны в обеих локализациях. Прямые POST контактной формы без точного согласия обрабатываются по утвержденному регламенту документа `14`. За модерацию, контроль сроков хранения (`retention`) и проверку отправок контактной формы (`submissions`) отвечает уполномоченный администратор ТОВ «Aerocool»; при смене сотрудника обязанность передается без перерыва.

Галочки по роли и текущим условиям Database фиксируют состояние на `2026-07-14`. Их нужно перепроверить при смене ответственного, тарифа, цен, лимитов или ожидаемой нагрузки. Они не подтверждают два отдельных технических пункта: `REVIEW_EMAIL_HASH_SALT` и миграции production-ветки Database.
