...
- `llms.txt`, chunking и похожие AI-тактики не должны опережать индексацию, качество контента и связный entity graph; текущий `llms.txt` указывает на ключевые публичные URL, все товарные страницы в `uk` и `ru`, и источники правды;
