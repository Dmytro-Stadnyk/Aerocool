...
| Что сравнить | Цель |
|---|---|
| `title` | Проектный заголовок должен быть точнее |
| H1 | Должен соответствовать интенту |
| Контент | Должен быть глубже и полезнее |
| Фото | Должны лучше подтверждать содержание и товар |
| Цена/наличие | Должно быть понятно |
| Schema.org | Разметка должна быть точнее и чище |
| Скорость | Ключевые показатели должны быть лучше без регрессий UX |
| Внутренние ссылки | Структура должна быть понятнее |
| Внешние ссылки на сайт | Нужно измерить отставание и качество источников |
| CTR | Нужно улучшать сниппет по данным Search Console |
