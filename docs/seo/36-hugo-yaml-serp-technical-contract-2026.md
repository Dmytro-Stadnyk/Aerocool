...
WebMCP и `llms.txt` не заменяют schema.org. WebMCP помогает совместимому браузеру и AI-агенту понять, какие действия можно выполнить на странице. `llms.txt` является добровольной картой для сторонних систем, которые решили его читать.

Google Search в обновлении от 2026-06-15 прямо указал, что `llms.txt` не нужен Google Search и не оказывает положительного или отрицательного влияния на видимость или ранжирование. Поэтому файл поддерживается только как вспомогательный экспериментальный интерфейс и не входит в набор сигналов, которыми проект обосновывает SEO-оценку.
